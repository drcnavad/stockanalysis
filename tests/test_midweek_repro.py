"""Regression: the LIVE engine (backtest_engine.winner_targets) reproduces the tested backtests exactly.
  ...
  3. C6-U96-T20-MW30 (the live rules from 2026-09-25: picks only from ranks 1-20, sector cap relaxed to fill 10 slots,
     top-3 swaps ignore the cap): engine == the independent re-implementation; the numbers are pinned as a regression
     (the rule itself was a user decision, not a pre-registered test)
...
# (exit_below, t20) -> (total return %, Sharpe) at 0.1%/side, and the never-seen 2022-04 -> 2024-09 Sharpe
EXPECTED = {(None, False): (438.08, 1.4649), (30, False): (421.59, 1.4695), (30, True): (414.07, 1.3716)}
EXPECTED_NEVER_SEEN = {(None, False): 0.8414, (30, False): 0.8172, (30, True): 0.5958}
MW = {"enter_top": 3, "exit_below": 15, "days": ["Mon", "Wed"]}
HAS_EXIT = "exit_all_below" in inspect.signature(be.winner_targets).parameters


def check(exit_all, t20=False):
    chk = []
    kw = {"exit_all_below": exit_all} if HAS_EXIT else {}
    kw["selection"] = dict(max_pick_rank=20, cap_soft=True) if t20 else g.PLAIN
    t_live, checks = be.winner_targets(g.sc, g.el, g.vol, g.reg, g.weekly, tiebreak_w=g.rs, check_log=chk, midweek=MW, **kw)
    t_test, swaps_test, sells_test = g.buffered_midweek(3, 15, exit_all, t20=t20)
    ...
    label = ("C6-U96-T20-MW" if t20 else "C6-U96-MW") + ("" if exit_all is None else str(exit_all))
    ...
    if EXPECTED[(exit_all, t20)] is not None:
        assert (round(m["Total Return %"], 2), round(m["Sharpe"], 4)) == EXPECTED[(exit_all, t20)], m
        assert round(ns["Sharpe"], 4) == EXPECTED_NEVER_SEEN[(exit_all, t20)], ns
...
swaps_test, sells_test = {(None, False): plain, (30, False): mw30, (30, True): t20}[(live_exit, live_t20)]
